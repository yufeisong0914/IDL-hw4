...
def test_greedy_decoding(generator=None, tokenizer=None):
    """Run all greedy search tests"""
    if generator is None or tokenizer is None:
        tokenizer = _make_default_tokenizer()
        generator = _make_default_generator()
    # Greedy Search Tests
...
def _make_default_tokenizer():
    """Create a default tokenizer for standalone test runs."""
    import importlib.util, sys, os
    # Load tokenizer module directly to avoid package import issues
    tok_path = os.path.join(os.path.dirname(__file__), '..', 'hw4lib', 'data', 'tokenizer.py')
    tok_path = os.path.normpath(tok_path)
    if 'hw4lib.data.tokenizer' not in sys.modules:
        spec = importlib.util.spec_from_file_location('hw4lib.data.tokenizer', tok_path)
        mod  = importlib.util.module_from_spec(spec)
        sys.modules['hw4lib.data.tokenizer'] = mod
        spec.loader.exec_module(mod)
    H4Tokenizer = sys.modules['hw4lib.data.tokenizer'].H4Tokenizer
    token_map = {
    ...
    return H4Tokenizer(token_map=token_map, token_type='char', validate=False)


def _make_default_generator():
    """Import SequenceGenerator, handling both package and standalone contexts."""
    import importlib.util, sys, os
    seq_path = os.path.join(os.path.dirname(__file__), '..', 'hw4lib', 'decoding', 'sequence_generator.py')
    seq_path = os.path.normpath(seq_path)
    if 'hw4lib.decoding.sequence_generator' not in sys.modules:
        # Ensure hw4lib.data.tokenizer is available first
        _make_default_tokenizer()
        spec = importlib.util.spec_from_file_location('hw4lib.decoding.sequence_generator', seq_path)
        mod  = importlib.util.module_from_spec(spec)
        sys.modules['hw4lib.decoding.sequence_generator'] = mod
        spec.loader.exec_module(mod)
    return sys.modules['hw4lib.decoding.sequence_generator'].SequenceGenerator


# No-argument wrappers for run_tests.py compatibility
def test_greedy_decoding_no_args():
    tokenizer = _make_default_tokenizer()
    SequenceGenerator = _make_default_generator()
    test_greedy_decoding(SequenceGenerator, tokenizer)
