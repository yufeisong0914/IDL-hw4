...
# -----------------------------------------------------------------------------------------------------------------------------------
# Wrapper for run_tests.py compatibility: takes LMDataset class and H4Tokenizer class
def test_lm_dataset(LMDataset, H4Tokenizer):
    token_map = {
    ...
    tokenizer = H4Tokenizer(token_map=token_map, token_type='char', validate=False)
    # config is the data sub-dict (same as notebook passes config['data'])
    data_config = {
        'root': './hw4_data_subset/hw4p1_data',
        'subset': None,
    }
    dataset = LMDataset(partition='train', config=data_config, tokenizer=tokenizer)
    test_dataset_lm(dataset)
