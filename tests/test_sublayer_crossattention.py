...
def test_forward_shapes(cross_attention):
    ...
    assert attn_weights.shape == (batch_size, num_heads, dec_seq_length, enc_seq_length), \
        f"Attention weights shape: expected {(batch_size, num_heads, dec_seq_length, enc_seq_length)} but got {attn_weights.shape}"
...
def test_padding_mask_behaviour(cross_attention):
    ...
    batch_size, num_heads, dec_seq_length, enc_seq_length = attn_weights.shape
    # 假设 to_pad 是从 enc_seq_length 开始的 padding index
    assert torch.all(attn_weights[:, :, :, to_pad:] == 0), "Attention weights for padded positions should be zero"
...
def test_cross_attention_behaviour(cross_attention):
    ...
    # attn_weights shape: (batch_size, num_heads, dec_seq_length, enc_seq_length)
    for dec_idx in range(dec_seq_length):
        other_positions = [j for j in range(enc_seq_length) if j != dec_idx]
        other_positions_attention = attn_weights[0, 0, dec_idx, other_positions]

        max_attention = attn_weights[0, 0, dec_idx, dec_idx]
        assert torch.all(max_attention > other_positions_attention), \
            f"Decoder position {dec_idx} does not attend most to its corresponding encoder position"
