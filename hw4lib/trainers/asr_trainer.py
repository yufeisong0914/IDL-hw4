...
class ASRTrainer(BaseTrainer):
    ...
    def __init__(self, model, tokenizer, config, run_name, config_file, device=None):
        ...
        # Initialize CE loss with pad token as ignore index and optional label smoothing
    ...
    def _train_epoch(self, dataloader):
        ...
        for i, batch in enumerate(dataloader):
            ...
            with torch.autocast(device_type=self.device, dtype=torch.float16):
                ...
                # Forward pass: seq_out (B, T, vocab), running_att, ctc_inputs
                ...
                # CE loss: reshape to (B*T, vocab) vs (B*T,)
                ...
                # CTC loss if needed
                if self.ctc_weight > 0:
                    # ctc_inputs['log_probs']: (T, B, vocab), ctc_inputs['lengths']: (B,)
                    ctc_log_probs = ctc_inputs['log_probs']       # (T, B, vocab)
                    ctc_lengths   = ctc_inputs['lengths'].to(self.device).long()
                    # targets for CTC: concatenated target token ids (without SOS/EOS padding)
                    # use targets_golden but remove padding
                    ctc_targets = targets_golden  # (B, max_tgt_len)
    ...
    def _validate_epoch(self, dataloader):
        ...
        results = self.recognize(dataloader, val_recognition_config, 'val_greedy')
        ...
        if len(references) == 0:
            return {'cer': 0.0, 'wer': 0.0, 'word_dist': 0.0}, results
